function Data = ConMaxSTARR(Data)
    Data.Optimisation.ConMaxSTARR.Weights = zeros(1, Data.Info.nAssets, Data.Info.nWindows);
    Data.Optimisation.ConMaxSTARR.CVaR = zeros(1, 1, Data.Info.nWindows);
    Data.Optimisation.ConMaxSTARR.VaR = zeros(1, 1, Data.Info.nWindows);
    for window = 1:Data.Info.nWindows
        ...
        A = [A;-rets -ones(S,1) -eye(S); c];
        b = [b; zeros(S,1); 0.0349];
        %b=[b;0.1*ones(S,1)];

        %solve, pass empty matrices for f, A, b, and ub
        [x, fval]=linprog(c,A,b,Aeq,beq,lb,[]);
        Data.Optimisation.ConMaxSTARR.CVaR(1, 1, window) = fval;
        Data.Optimisation.ConMaxSTARR.Weights(1, :, window) = x(1:20)';
        Data.Optimisation.ConMaxSTARR.VaR(1, 1, window) = x(21);
    end
