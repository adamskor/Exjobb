function [Data] = FormatData(Data)
    ...
    volIndex =  T_matrix2(3:4755, 5)/100;
    ...
    for t = 1:size(prices,2)
        ...
        index = flip(index);
        volIndex = fillmissing(volIndex, 'linear'); 
        volIndex = flip(volIndex);
        rf_rate= fillmissing(rf_rate, 'linear'); 
        rf_rate_temp = flip(rf_rate);
        
    end
    %prices = prices(1:1080, :);
    ...
    volIndex_returns = zeros(window_size, 1, size(prices,1)-1 -window_size);
    ...
    volIndex_returns_temp = zeros(size(prices,1)-1, 1);
    ...
    for asset = 1:size(prices,2)
        for t = 2:size(prices,1)
            ...
            volIndex_returns_temp(t-1, 1) = (volIndex(t) - volIndex(t-1))/volIndex(t-1);
    ...
    for i = window_size:size(returns_temp,1)
        ...
        volIndex_returns(1:window_size, :, window) = volIndex_returns_temp(i-(window_size-1):i, :);
        rf_rate(1:window_size, :, window) = rf_rate_temp(i-(window_size-1):i, :);
        
        window = window + 1;
    end
    
    Data.TimeSeries.allReturns = returns_temp - rf_rate_temp(1:max(size(prices)) - 1);
    
    Data.TimeSeries.Prices = prices;
    Data.TimeSeries.RFrate = rf_rate/360;
    Data.TimeSeries.MSCI = index_returns - Data.TimeSeries.RFrate;;
    Data.TimeSeries.volIndex = volIndex_returns;
    ...
    Data.TimeSeries.MSCIeps = index_returns - mean(index_returns);
    Data.TimeSeries.volIndexeps = volIndex_returns - mean(volIndex_returns);
